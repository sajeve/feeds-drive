...
import dh.newspaper.model.json.JsonCategory;
	...
	private void writeJson() throws IOException {
		{
			...
			mapper.writerWithDefaultPrettyPrinter().writeValue(f, new JsonCategory[]{
					new JsonCategory(0, "Tổng hợp"),
					new JsonCategory(1, "Thời sự"),
					new JsonCategory(2, "Du lich"),
					new JsonCategory(3, "Thế giới")
			});
		}
		{
	...
	private void readJson() throws IOException {
		{
			...
			assertEquals("Tổng hợp", categories.get(0).getName());
		}
		{
			File f = new File(PATHTOCONTENT_FILE);
		...
		}
		{
			...
			assertEquals(1L, subscriptions.get(0).getCategories().get(0).longValue());
			assertEquals(3L, subscriptions.get(0).getCategories().get(1).longValue());
			assertEquals("en-US", subscriptions.get(0).getLanguage());
			assertTrue( subscriptions.get(0).isEnable());
			assertEquals(0L, subscriptions.get(4).getCategories().get(0).longValue());

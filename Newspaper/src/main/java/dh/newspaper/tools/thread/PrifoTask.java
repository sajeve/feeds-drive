...
/**
 * A {@link dh.newspaper.tools.thread.PrifoTask} must implement {@link #run()} which constantly check for {@link #isCancelled()}
 ...
 * <p></p>
 *
 * A {@link dh.newspaper.tools.thread.PrifoTask} must also implement missionId. Two task of the same missionId are twin
 * if we add task in the queue which already contains the twin so the task will not be added, but the priority of the twin
 * will be increased. See {@link dh.newspaper.tools.thread.PrifoQueue#offer(IPrifosable)}
 *

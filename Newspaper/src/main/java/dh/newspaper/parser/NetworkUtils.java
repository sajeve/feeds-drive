...
public class NetworkUtils {
	 ...
	 * @throws IOException
	 */
	public static InputStream getStreamFromUrl(String address) throws IOException {
		Stopwatch sw;
		if (Constants.DEBUG) {
		...
		AndroidHttpClient client = AndroidHttpClient.newInstance(MOBILE_USER_AGENT);
		...
		try {
			...
			if (Constants.DEBUG) {
				sw = Stopwatch.createStarted();
				Log.v(TAG, "Downloaded OK ("+sw.elapsed(TimeUnit.MILLISECONDS)+" ms) "+address);
			}
		}
	}
}

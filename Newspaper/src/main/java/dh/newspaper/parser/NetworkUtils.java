...
//	}
	...
	public static InputStream getStreamFromUrl(String address, String userAgent) throws IOException {
		...
		if (Constants.DEBUG) {
			sw = Stopwatch.createStarted();
			Log.v(TAG, "DownloadingOh " + address);
		}
		...
		try {
			httpConnection = client.open(new URL(address));
			...
			if (Constants.DEBUG) {
				...
				Log.v(TAG, "DownloadedOh OK "+size+" bytes ("+sw.elapsed(TimeUnit.MILLISECONDS)+" ms) "+address);
			}
		}
	}

...
public class ReadabilityExtractor {
	...
	public Element fetchArticleContent(Document document, String lang) {
        ...
        if(topCandidate != null) {
        	articleContent = findSiblingElements(topCandidate, document, lang);
        }
	...
	private Element findSiblingElements(Element topCandidate, Document document, String lang) {
		/*Element articleContent = document.createElement("div");
		 ...
		 if(siblingNodes.size() > 0) {
			 for (Element siblingNode : siblingNodes) {
					...
					topCandidate = AddSiblings.addSiblings(topCandidate, lang);
				}
